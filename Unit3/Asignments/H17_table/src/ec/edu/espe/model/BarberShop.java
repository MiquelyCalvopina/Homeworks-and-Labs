...
public class BarberShop {
     ...
     */
    public int newCustomer(String dataInput) throws IOException{
        String auxString;
        List<String> dataSplit = Arrays.asList(dataInput.split(";"));
        auxString = searchCostumer(dataSplit.get(0));
        if(auxString == "0")
        {
            FileManager.writeFile(new File("Clientes.csv"), dataInput);
            return 0;
        }
        else
        {
     ...
     */
       public void modifyCostumer(Person costumer, String identificationCard) throws IOException {
        FileManager.modifyFile(new File("Clientes.csv"), identificationCard, datainput());
    }
    ...
    public String datainput() throws IOException {
       ...
       boolean aux;
        ...
        System.out.print("Ingrese su nombre y apellido: ");
        scan.skip("\n");
        String fullNameNormal = dataInput.nextLine();
        String fullName=fullNameNormal.toUpperCase();
    ...
    */
    ...
    public String inText(Service service , boolean ModifyOrNewVerification) throws IOException {
        ...
        do {            
           ...
           return service.generateCode()+ ";"+ service.getFullName().toUpperCase()+";"+service.getCost()+"\r\n";
        }
        else
        {
    ...
    public void deleteCustomer(File file, ArrayList<Customer> customer, String CI) {
		String letter = "";
                FileManager.createCustomerArray(customer, file);
		try {
			BufferedWriter bw = new BufferedWriter(new FileWriter(file));
			for (int k = 0; k < customer.size(); k++) {
				if (customer.get(k).getIdentifyCardNumber().equals(CI)) {
                                    System.out.println("Se encontro el cliente");
                                    System.out.println("El cliente " + customer.get(k).getIdentifyCardNumber() + " con cedula de identidad "
                                    		+ customer.get(k).getFullName() + " ha sido eliminado del registro");
				} else {
    ...
    public void deleteEmployee(String file, ArrayList<Employee>employees, String ic) throws IOException {
     ...
     public  boolean validateCustomer(ArrayList<Customer> customer, File file, String CI) {
		boolean validateCI = false;
		FileManager.createCustomerArray(customer, file);
		for (int i = 0; i < customer.size(); i++) {
			if (customer.get(i).getIdentifyCardNumber().equals(CI)) {
				validateCI = true;
			}
		}
		customer.clear();
